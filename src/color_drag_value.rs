...
impl<'a> Widget for ColorDragValue<'a> {
    fn ui(self, ui: &mut Ui) -> Response {
        ...
        let mut response = if is_kb_editing {
            ...
            .fill(if value >= 0.0 {
                Rgba::from_rgb(0.0, (value / range.end()).min(1.0) as _, 0.0)
            } else {
                Rgba::from_rgb((value / range.start()).max(-1.0) as _, 0.0, 0.0)
            })
